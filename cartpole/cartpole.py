...
https://all.cs.umass.edu/rlr/domains.html
...
def choose_action(state, mdp_data):
    ...
    nagrada_nula = np.dot(mdp_data['transition_probs'][state, :, 0], mdp_data['value']) 
    nagrada_jedan = np.dot(mdp_data['transition_probs'][state, :, 1], mdp_data['value'])

    if nagrada_nula > nagrada_jedan:
        return 0
    elif nagrada_jedan > nagrada_nula:
        return 1
    else:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for stanje in range(mdp_data['num_states']):
        stc0 = transition_counts[stanje, :, 0]
        stc1 = transition_counts[stanje, :, 1]
        suma0 = 0
        suma1 = 0
        for i in range(len(stc0)):
            suma0 += stc0[i]
            suma1 += stc1[i]
        if suma0 != 0:
            mdp_data['transition_probs'][stanje, :, 0] = np.true_divide(stc0, suma0)
        if suma1 != 0:
            mdp_data['transition_probs'][stanje, :, 1] = np.true_divide(stc1, suma1)
        if reward_counts[stanje, 1] != 0:
            mdp_data['reward'][stanje] = np.true_divide(reward_counts[stanje, 0], reward_counts[stanje, 1])
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    br_iteracija = 0
    pre = None
    posle = mdp_data['value']
    for iteracija in range(100):

        pre = posle 
        akcije0 = np.dot(mdp_data['transition_probs'][:, :, 0], pre)
        akcije1 = np.dot(mdp_data['transition_probs'][:, :, 1], pre)
        akcijemax = np.zeros(akcije0.shape)

        for i in range(len(akcije0)):
            if akcije0[i] >= akcije1[i]:
                akcijemax[i] = akcije0[i]
            else:
                akcijemax[i] = akcije1[i]

        posle = mdp_data['reward'] + gamma * akcijemax

        if np.linalg.norm(posle - pre) < tolerance:
            br_iteracija = iteracija + 1
            break
    
    mdp_data['value'] = posle 
