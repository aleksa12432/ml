...
def update_W(W, x, learning_rate):
    ...
    # *** START CODE HERE ***
    updated_W = W
    sgnwx = np.sign(W.dot(x))
    sgnwxxt = np.zeros(W.shape)
    for i in range(len(sgnwxxt)):
        for j in range(len(sgnwxxt[0])):
            sgnwxxt[i][j] = x[j] * sgnwx[i]
    updated_W += learning_rate * (np.linalg.pinv(W.T) - sgnwxxt) 
    # *** END CODE HERE ***
